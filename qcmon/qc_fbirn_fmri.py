...
#         self.calc_values()
#         self.varI = self.make_diff_img()
#         self.meanI = self.make_ave_img()
#
#     # May want to add drift and sd calc from make_plots_file here...
#
#
#     def get_roi_width(self):
#         if self.num_pix == 128:
...
#
#
#     def make_diff_img(self):
#         Isub = self.Iodd - self.Ieven
#         img = Isub.reshape((self.num_pix, self.num_pix)).T
#         sub = img[self.X1:self.X2, self.X1:self.X2]
#         return np.var(sub.flatten(), ddof=1)
#
#
#     def make_ave_img(self):
#         Sy = self.Iodd + self.Ieven
#         self.Iave = Sy / self.num_TRs
#         img = Iave.reshape((self.num_pix, self.num_pix)).T
#         sub = img[self.X1:self.X2, self.X1:self.X2]
#         return sub.flatten().mean()
...
# sfnrI is an accidentally renamed Isfnr below?
# make_plots needed inputs:
#   - sd, drift
#   - x, roi, x, yfit
#   - f, vals, meanI, snr, sfnrI
#   - rr, F, fcalc, rdc
#   - output_prefix

# make_csv needed inputs:
#   - meanI, y, sd, drift, snr, sfnrI, rdc
#   - output_prefix


def make_images_plot(phantom, output_prefix):
...
# Have to come back here and make sure all references to phantom. are defined
...
def make_plots(N, roi, roir, r2, meanI, snr, sfnrI, output_prefix,
    ...
    fig.suptitle(
        f"Percent fluct. (trend removed), drift = {sd:5.2f} "
        f"{drift:5.2f}"
    )
